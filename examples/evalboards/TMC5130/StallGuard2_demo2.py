...
def stallguard2_init(mc, motor, eval_board, init_velocity):
    print(eval_board.read_register_field(mc.FIELD.SGT))
    print(eval_board.read_register_field(mc.FIELD.SG_STOP))
    print(eval_board.read_register_field(mc.FIELD.TCOOLTHRS))
    ...
    v_max=round(53687/pow(2, Microstep_resolution)*init_velocity)           # internal unit  <- rps
...
def main():
    ...
    with ConnectionManager().connect() as my_interface:
        ...
        eval_board.write_register_field(mc.FIELD.MRES, 5)  # 2^MRES = 2^5 = 32 micro-steps
