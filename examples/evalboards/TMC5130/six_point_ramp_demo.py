...
with ConnectionManager().connect() as my_interface:
    ...
    while motor.actual_position != 0:
        ...
        print(f"time: {values_time[i]:.1f} s\tActual position: {values_position[i]} \t Actual speed: {values_speed[i]}")
        i+=1
        time.sleep(T_s)
    ...
    plot.plot(x, values_position)
    plot.xlabel ('Time [s]')
    plot.ylabel ('Position [Microsteps]')
    plot.title ('Position')
    ...
    fig, ax2 = plot.subplots(num=2, clear=True)
    plot.plot(x, values_speed)
    plot.xlabel ('Time [s]')
    plot.ylabel ('Speed [Microsteps / second]')
 ...
 #calculating the acceleration
    ...
    for ii in values_speed:
        ...
        #print(str(values_acceleration[m]))   #for printing the acceleration
        m += 1
    values_acceleration[0]=values_acceleration[1] #first speed value is incorrect. Therefore it is replaced

    # 3. Plot: acceleration
    fig, ax3 = plot.subplots()
    plot.plot(x, values_acceleration)
    plot.xlabel ('Time [s]')
    plot.ylabel ('Acceleration [Microsteps /s^2]')
