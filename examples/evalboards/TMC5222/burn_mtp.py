...
TMC5222.REGMAP.MTP_STATUS = Register(name="MTP_STATUS", parent=TMC5222.REGMAP, access=Access.RW, address=0x0011, signed=False)
TMC5222.REGMAP.MTP_CP_CONTROL2 = Register(name="MTP_CP_CONTROL2", parent=TMC5222.REGMAP, access=Access.RW, address=0x0021, signed=False)
TMC5222.REGMAP.MTP_CP_STATUS = Register(name="MTP_CP_STATUS", parent=TMC5222.REGMAP, access=Access.RW, address=0x0022, signed=False)
TMC5222.REGMAP.MTP_DATA0 = Register(name="MTP_DATA0", parent=TMC5222.REGMAP, access=Access.RW, address=0x0030, signed=False)
TMC5222.REGMAP.MTP_DATA1 = Register(name="MTP_DATA1", parent=TMC5222.REGMAP, access=Access.RW, address=0x0031, signed=False)
TMC5222.REGMAP.MTP_DATA2 = Register(name="MTP_DATA2", parent=TMC5222.REGMAP, access=Access.RW, address=0x0032, signed=False)
TMC5222.REGMAP.MTP_DATA3 = Register(name="MTP_DATA3", parent=TMC5222.REGMAP, access=Access.RW, address=0x0033, signed=False)
TMC5222.REGMAP.MTP_ADDR = Register(name="MTP_ADDR", parent=TMC5222.REGMAP, access=Access.RW, address=0x0034, signed=False)
...
# Table 23 "MTP Record Types" in the data sheet
...
def check_burning_voltage():
    # Check if the burning voltage is within range - Programming Voltage (VM): 8.7V ± 0.13V (1.5%)
    for x in range(0, 3):
        comparator_value = tmc5222_eval.read(TMC5222.REGMAP.MTP_CP_STATUS) & 0x7
...
def set_data_to_burn(record_type, data_to_program):
    ...
    tmc5222_eval.write(TMC5222.REGMAP.MTP_DATA0, data_to_program)
    data_to_program = data_to_program >> 8
    tmc5222_eval.write(TMC5222.REGMAP.MTP_DATA1, data_to_program)
    data_to_program = data_to_program >> 8
    tmc5222_eval.write(TMC5222.REGMAP.MTP_DATA2, data_to_program)
    data_to_program = data_to_program >> 8
    tmc5222_eval.write(TMC5222.REGMAP.MTP_DATA3, data_to_program)
    # Write the record type
    tmc5222_eval.write(TMC5222.REGMAP.MTP_ADDR, record_type)

def burn_data():
    # Check if there is space left in otp
    mtp_status = tmc5222_eval.read(TMC5222.REGMAP.MTP_STATUS)
    mtp_full = (mtp_status & 0x4) >> 2
    if not mtp_full:
        tmc5222_eval.write(TMC5222.REGMAP.MTP_CONTROL, 0x1)
        done_burning = 0
        vpp_init_fail = 0
        ...
        if done_burning:
           print("DONE_BURNING: Record burned.")
    else:
...
def restore_otp():
    ...
    tmc5222_eval.write(TMC5222.REGMAP.MTP_CONTROL, 0x80)
...
with ConnectionManager().connect() as my_interface:
    ...
    # Set the default I2C address of the Landungsbrücke
    lb_i2c_address(my_interface, 0b1100000) 

    # Check for internal oscillator
    oscillator = tmc5222_eval.read(TMC5222.REGMAP.GCR_IOIN.EXT_CLK)
    ...
    tmc5222_eval.write(TMC5222.REGMAP.MTP_CP_CONTROL2, 0x1B)

    # Programming Voltage (VM): 8.7 ± 0.13V (1.5%)
    ...
    # Leave OTP_MODE
    tmc5222_eval.write(TMC5222.REGMAP.MTP_CP_CONTROL2, 0x0)
