...
# Note the moving average filter already effects the "actual_velocity_rpm_notfiltered" / VELOCITY_PER.
moving_average_filter = 0  # The allowed value ranges from 0..7
...
with cm.connect() as my_interface:
    ...
    # Meter type PER is also the default already but we set it in case it was changed before.
    tmc9660_device.write(TMC9660.MCC.VELOCITY_CONFIG.METER_TYPE.choice.VELOCITY_PER)
    # Note the moving average filter already effects the "actual_velocity_rpm_notfiltered" / VELOCITY_PER.
    tmc9660_device.write(TMC9660.MCC.VELOCITY_CONFIG.MOVING_AVRG_FILTER_SAMPLES, moving_average_filter)

    filter = NormalizedBiquadFilterCoefficients(a_1=-26026, a_2=1022549, b_0=13013, b_1=26027, b_2=13013)

    tmc9660_device.write(TMC9660.MCC.BIQUAD_V_A_1, filter.a_1)
    tmc9660_device.write(TMC9660.MCC.BIQUAD_V_A_2, filter.a_2)
    tmc9660_device.write(TMC9660.MCC.BIQUAD_V_B_0, filter.b_0)
...
ax.plot(t, v_filtered, label="velocity-filtered")
ax.legend()
plt.show()
