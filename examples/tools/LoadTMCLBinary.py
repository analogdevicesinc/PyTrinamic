...
tmcl_file_path = "test.bin"   # This can be any TMCL binary file generated by the TMCL-IDE or by TMCLAsm

with open(tmcl_file_path, "rb") as file:
    ...
    with connection_manager.connect() as my_interface:
        ...
        for cmd_array in iter(partial(file.read, 8), b""):
            my_interface.send(cmd_array[0], cmd_array[1], cmd_array[2], (cmd_array[3]<<24)|(cmd_array[4]<<16)|(cmd_array[5]<<8)|(cmd_array[6]<<0))
       
        my_interface.send(TMCLCommand.QUIT_DOWNLOAD_MODE, 0, 0, 0)
        my_interface.send(TMCLCommand.RESET_APPLICATION, 0, 0, 0)
        my_interface.send(TMCLCommand.RUN_APPLICATION, 0, 0, 0)    # This can be omitted if the program does not need to be started right after downloading
