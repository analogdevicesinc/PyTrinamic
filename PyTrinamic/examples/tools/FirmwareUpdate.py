...
reply = myInterface.send(TMCL.COMMANDS["BOOT_GET_INFO"], 0, 0, 0)
mem_page_size = reply.value
reply = myInterface.send(TMCL.COMMANDS["BOOT_GET_INFO"], 1, 0, 0)
mem_start_address = reply.value
reply = myInterface.send(TMCL.COMMANDS["BOOT_GET_INFO"], 2, 0, 0)
...
reply = myInterface.send(TMCL.COMMANDS["BOOT_ERASE_ALL"], 0, 0, 0)
...
def writePage(page):
    ...
    myInterface.send(TMCL.COMMANDS["BOOT_WRITE_PAGE"], 0, 0, current_page)
...
def write32Bit(address, writeData):
    ...
    myInterface.send(TMCL.COMMANDS["BOOT_WRITE_BUFFER"], math.floor(offset/4) % 256, math.floor(math.floor(offset/4) / 256), writeData)
...
reply = myInterface.send(TMCL.COMMANDS["BOOT_GET_CHECKSUM"], 0, 0, end_address-1)
...
myInterface.send(TMCL.COMMANDS["BOOT_WRITE_LENGTH"], 0, 0, length)
# Write firmware checksum
myInterface.send(TMCL.COMMANDS["BOOT_WRITE_LENGTH"], 1, 0, checksum)

# Restart the firmware
print("Starting the firmware")
myInterface.send(TMCL.COMMANDS["BOOT_START_APPL"], 0, 0, 0)
