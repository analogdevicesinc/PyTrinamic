...
class TMCM_1140(tmcl_module):
    ...
    class motor_0(tmcl_module.Motor, DriveSettingModule, LinearRampModule,MotorControlModule, StallGuard2Module, CoolStepModule):
        ...
        class APs():
            ...
            EncoderPosition	            =	209	#	The current encoder position
            EncoderPrescaler	        =	210	#	Prescaler of the encoder.
            MaxEncoderDeviation	        =	212	#	When the actual position (parameter 1) and the encoder position (parameter 209) differ more than set here the motor will be stopped. This function is switched off when the maximum deviation is set to zero.
            ...
            AbsoluteResolverValue	    =	215	#	Absolute position of the internal sensOstep encoder. The absolute position is within one motor rotation.
            ExternalEncoderPosition	    =	216	#	The value of the external encoder register can be read out or written. This parameter is only used if an external encoder is connected.
            ExternalEncoderPrescaler	=	217	#	Prescaler for external encoder.
            ExternalEncoderMaxDeviation	=	218	#	When the actual position (parameter #1) and the external encoder position (parameter #216) differ more than set here the motor will be stopped. Setting this parameter to zero turns off this function.
