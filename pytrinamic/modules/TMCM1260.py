...
class TMCM1260(TMCLModule):
    """
    The TMCM-1260 is a single axis controller/driver module. Supply voltage is 24V.
    """
    def __init__(self, connection, module_id=1):
        super().__init__(connection, module_id)
        self.name = "TMCM-1260"
        self.desc = self.__doc__
        self.motors = [self._MotorType(self, 0)]
    ...
    def move_by(self, axis, delta, velocity=None):
        if velocity:
            self.motors[axis].linear_ramp.max_velocity = velocity
        return self.connection.move_by(axis, delta, self.module_id)

    class _MotorType(MotorControlModule):
